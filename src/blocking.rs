...
/// Synchronous client for interacting with the Dune Analytics API.
///
/// This is a blocking version of the async [`DuneClient`](crate::DuneClient) that can be used
...
impl DuneClient {
    /// Creates a new synchronous Dune client with the given API key.
    ///
    /// # Arguments
    ///
    ...
    /// Creates a new synchronous Dune client with a custom base URL.
    ...
    ///
    /// let client = DuneClient::new("your-api-key").unwrap();
    /// let response = client.execute_sql("SELECT * FROM ethereum.transactions LIMIT 10").unwrap();
    /// println!("Execution ID: {}", response.execution_id);
    /// ```
    pub fn execute_sql(&self, sql: impl Into<String>) -> Result<ExecuteResponse> {
        let request = ExecuteSqlRequest {
            sql: sql.into(),
            ..Default::default()
        };
        self.execute_sql_with_options(request)
    }

    /// Executes an arbitrary SQL query with additional options.
    pub fn execute_sql_with_options(&self, request: ExecuteSqlRequest) -> Result<ExecuteResponse> {
    ...
    ///
    /// let client = DuneClient::new("your-api-key").unwrap();
    /// let response = client.execute_query(1234567).unwrap();
    /// println!("Execution ID: {}", response.execution_id);
    /// ```
    pub fn execute_query(&self, query_id: u64) -> Result<ExecuteResponse> {
    ...
    ///
    /// let client = DuneClient::new("your-api-key").unwrap();
    /// let status = client.get_execution_status("01234567-89ab-cdef-0123-456789abcdef").unwrap();
    ...
    ///
    /// * `execution_id` - The execution ID returned from an execute call.
    pub fn get_execution_results(&self, execution_id: &str) -> Result<ExecutionResultsResponse> {
    ...
    pub fn get_execution_results_with_options(
        &self,
        execution_id: &str,
    ...
    ///
    /// * `execution_id` - The execution ID returned from an execute call.
    pub fn get_execution_results_csv(&self, execution_id: &str) -> Result<String> {
    ...
    pub fn get_execution_results_csv_with_options(
        &self,
        execution_id: &str,
        options: ResultOptions,
    ) -> Result<String> {
    ...
    ///
    /// let client = DuneClient::new("your-api-key").unwrap();
    /// let results = client.get_latest_results(1234567).unwrap();
    /// if let Some(result) = results.result {
    ///     println!("Got {} rows", result.rows.len());
    /// }
    /// ```
    pub fn get_latest_results(&self, query_id: u64) -> Result<ExecutionResultsResponse> {
    ...
    pub fn get_latest_results_with_options(
        &self,
        query_id: u64,
        options: ResultOptions,
    ) -> Result<ExecutionResultsResponse> {
    ...
    ///
    /// * `query_id` - The ID of the saved query.
    pub fn get_latest_results_csv(&self, query_id: u64) -> Result<String> {
    ...
    pub fn get_latest_results_csv_with_options(
        &self,
        query_id: u64,
        options: ResultOptions,
    ) -> Result<String> {
    ...
    ///
    /// let client = DuneClient::new("your-api-key").unwrap();
    /// let response = client.cancel_execution("01234567-89ab-cdef-0123-456789abcdef").unwrap();
    /// if response.success {
    ///     println!("Execution cancelled successfully");
    /// }
    /// ```
    pub fn cancel_execution(&self, execution_id: &str) -> Result<CancelExecutionResponse> {
    ...
    pub fn run_sql(
        ...
        let execute_response = self.execute_sql(sql)?;
        self.wait_for_results(&execute_response.execution_id, timeout)
    }

    /// Executes a saved query and waits for the results.
    pub fn run_query(&self, query_id: u64, timeout: Duration) -> Result<ExecutionResultsResponse> {
        let execute_response = self.execute_query(query_id)?;
        self.wait_for_results(&execute_response.execution_id, timeout)
    }
    ...
    pub fn wait_for_results(
        ...
        loop {
            ...
            let status = self.get_execution_status(execution_id)?;
    ...
    fn handle_text_response(&self, response: reqwest::blocking::Response) -> Result<String> {
        let status = response.status();
